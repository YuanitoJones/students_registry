...
const CardComponent = React.forwardRef<HTMLElement, ICard>(function SimpleCard(props, _ref) {
   const { title, description, body, footer, ...rest } = props;
   return (
      <Card.Root {...rest}>
         {title && (
            <Card.Header>
               <Card.Title {...title}>{title.children}</Card.Title>
               <Card.Description {...description}>{description?.description}</Card.Description>
            </Card.Header>
         )}
         <Card.Body {...body}>{body?.children}</Card.Body>
         <Card.Footer {...footer}>{footer?.children}</Card.Footer>
      </Card.Root>
   );
});
